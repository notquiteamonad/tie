...
module TIE.Data.Config.Internal.TH where
...
import TIE.Data.Config.Internal (Config)
...
makeConfigLenses name = do
  ...
  where
    ...
    makeConfigLens (constructorName, constructorType) acc = do
      ...
      InfixE wrapped compose _ <- [e|_Wrapped . x|]
      let sig = SigD lensName (AppT (AppT (ConT ''Lens') (ConT ''Config)) constructorType)
          impl = ValD (VarP lensName) (NormalB $ InfixE wrapped compose (Just $ VarE versionedLensName)) []
      pure $ acc <> [sig, impl]
